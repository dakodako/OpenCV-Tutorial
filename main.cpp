...
int main( int argc, char** argv )
{
    ...
    // Load the images
   Mat image1= imread( "correctionresultiii.jpg" );
   Mat image2= imread( "correctionresulti.jpg");
 ...
 //   Rect myRoi1(108,62,215,320);
   // Rect myRoi2(103,300,818,468);
    
    /*image1 = image1(myRoi2);
    image2 = image2(myRoi2);
   ...
   cvtColor( image2, gray_image2, CV_RGB2GRAY );
    ...
    std::vector< DMatch > good_matches;
       for( int i = 0; i < descriptors_object.rows; i++ ){
        ...
        if( matches[i].distance < 5*min_dist  ){//2.5 0.02 works for putting them together on 1920 * 1080
            ...
            if(std::abs(slope) < 0.01){ //3,0.015 warp succeed for 1024 * 768
                if(keypoints_object[matches[i].queryIdx].pt.x < largest2){
                    largest2 = keypoints_object[matches[i].queryIdx].pt.x;
                   // largest1 = i;
                }
                if(keypoints_object[matches[i].queryIdx].pt.x > largest){
                    largest = keypoints_object[matches[i].queryIdx].pt.x;
                }
                if(keypoints_scene[matches[i].trainIdx].pt.x > largest1){
                    largest1 = keypoints_scene[matches[i].trainIdx].pt.x;
                    //largest2 = i;
                }
                if(keypoints_scene[matches[i].trainIdx].pt.x < smallest){
                    smallest = keypoints_scene[matches[i].trainIdx].pt.x;
                }
   ...
   // printf("homo %f\n ",H.at<double>(3,0));
    printf("width of H %d\n",H.size().width);
    printf("height of H %d\n",H.size().height);
    
    
    /*for(m = 1; m < H.size().width;m++){
        for(n = 1; m < H.size().height;n++){
            printf("%d %d %f\n",m,n,H.at<double>(m,n));
        }
    }*/
